...
At the moment this is the only workflow that is fully automated, and we are very proud of it, since it saves us a lot of time and allows us to provide the best experience for our Linux users. We are planning to support more Linux distributions in the future, and we will continue to improve our [CI](https://en.wikipedia.org/wiki/Continuous_integration) workflow to make it even more efficient and reliable.
The only manual step is to promote the new release from the alpha channel to the stable channel, but we have a dedicated [CI](https://en.wikipedia.org/wiki/Continuous_integration) workflow for that which needs to be triggered manually. And this is quite a simple and easy process.
...
The Snap package is built and tested in our [CI](https://en.wikipedia.org/wiki/Continuous_integration) workflow and is also completely automated. The only manual action required is to promote the new release from the beta channel to the stable channel, but this is just a few clicks in the Snapcraft dashboard.
...
Similar to Linux, during the [CI](https://en.wikipedia.org/wiki/Continuous_integration) workflow we run a set of tests to make sure the installer can be installed and works as expected on the latest Windows version for both x64 and arm64 architectures. These tests cover not only fresh installations but also updates from a previous version.
